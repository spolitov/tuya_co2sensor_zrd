...
u16 load_le16(const u8* start) {
  return *start + ((u16)start[1] << 8);
}

void store_be16(u8* out, u16 input) {
  out[0] = input >> 8;
  out[1] = input & 0xff;
}

void store_le16(u8* out, u16 input) {
  out[0] = input & 0xff;
  out[1] = input >> 8;
}
...
static void publish_co2_last_calibration(void* arg) {
  ...
  report_t report;
  TL_SETSTRUCTCONTENT(report, 0);
  report.attr[0].attrID = ZCL_CO2_MEASUREMENT_ATTRID_LAST_CALIBRATION;
  report.attr[0].dataType = ZCL_DATA_TYPE_UTC;
  report.attr[0].attrData = (u8*)&attributes.co2_last_calibration;
  report.numAttr = 1;
  ...
  zcl_sendReportAttrsCmd(APP_ENDPOINT1, &dstEpInfo, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR, ZCL_CLUSTER_MS_CO2_MEASUREMENT, (zclReportCmd_t*)&report);
}
  ...
  if (len == 4) {
    if (load_le16(data) == 0x0116 && load_le16(data + 2) == 0xe603) {
      if (last_calibration_request_time) {
        attributes.co2_last_calibration = last_calibration_request_time;
...
void start_calibration(u32 time) {
  u16 value = attributes.co2_calibration_value;
...
void user_init(bool isRetention) {
  ...
  bdb_defaultReportingCfg(APP_ENDPOINT1, HA_PROFILE_ID, ZCL_CLUSTER_MS_CO2_MEASUREMENT,
                          ZCL_CO2_MEASUREMENT_ATTRID_MEASUREDVALUE, 10, 300, (u8 *)&reportableChange);
  bdb_defaultReportingCfg(APP_ENDPOINT1, HA_PROFILE_ID, ZCL_CLUSTER_MS_TEMPERATURE_MEASUREMENT,
                          ZCL_TEMPERATURE_MEASUREMENT_ATTRID_MEASUREDVALUE, 10, 300, (u8 *)&reportableChange);
  bdb_defaultReportingCfg(APP_ENDPOINT1, HA_PROFILE_ID, ZCL_CLUSTER_MS_RELATIVE_HUMIDITY,
                          ZCL_ATTRID_HUMIDITY_MEASUREDVALUE, 10, 300, (u8 *)&reportableChange);
  /* Initialize BDB */
